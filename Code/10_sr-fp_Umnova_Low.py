...
class ExactSolutionHarmonic(UserExpression):
    def __init__(self, t, L0, Li, L1, velf, velp, omegaf, omegap, rhof, rhop, phi, gamma, sigma, **kwargs):
        ...
        a = np.array([[np.exp(-1j * kf * L0), np.exp(1j * kf * L0), 0, 0],
                      [0, 0, np.exp(-1j * kp * L1), np.exp(1j * kp * L1)],
...
probIdStr = 'sr-fp-UmnovaLow' # Identification of the problem for save files' names
...
rigid_boundary = CompiledSubDomain("on_boundary && (near(x[0], L1))", L1=L1)
...
def compute(Nele, dt, T_init, T_final, initCondId = 7, savePrefix=''):
    ...
    rigid_boundary.mark(boundary_markers, 3)    # rigid boundary
    ...
    bc_rigid = DirichletBC(V, zero, boundary_markers, 3)
