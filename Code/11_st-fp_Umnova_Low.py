...
probIdStr = 'st-fp-UmnovaLow' # Identification of the problem for save files' names
...
sigma_porous = 10000. #100#27888 Flux resistivity [N.s/m^4]
...
def compute(Nele, dt, T_init, T_final, initCondId = 7, savePrefix=''):
    ...
    domain_markers.set_all(1)  # all elements are set to zero
    ...
    zero = Constant("0.0")
    if initCondId == 7:  # Harmonic 6
        speakerOsc = Expression('sin(omega*t)', omega=omega_fluid, t=0., degree=1)
    elif initCondId == 6:  # Mexican hat 7
...
if __name__ == '__main__':
    ...
    print('Test error: %g' % compute(Nele, dt, T_init, T_final, savePrefix='_fade_'))
