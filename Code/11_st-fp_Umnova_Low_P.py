...
class ExactSolutionHarmonic(UserExpression):
    def __init__(self, t, L0, Li, L1, velf, velp, omegaf, omegap, rhof, rhop, phi, gamma, sigma, **kwargs):
        self.t = t
        self.L0 = L0
        self.Li = Li
        self.L1 = L1
        self.velf = velf; self.velp = velp
        self.omegaf = omegaf; self.omegap = omegap
        self.rhof = rhof; self.rhop = rhop
        self.phi = phi
        self.gamma = gamma
        self.kf = omegaf / velf
        self.kp = np.sqrt(phi * gamma / (rhop * velp ** 2) * (omegap ** 2 * rhop + 1j * omegap * sigma))
        kf = self.kf; kp = self.kp
        r1 = 1j * kf * rhof * velf ** 2
        r2 = 1j * kp * rhop * velp ** 2 / (phi ** 2 * gamma)
        r3 = kp * rhop * velp ** 2 / (rhof * velf * phi ** 2 * gamma)
        a = np.array([[np.exp(-1j * kf * L0), np.exp(1j * kf * L0), 0, 0],
                      [0, 0, -1j * (r3 + omegap) * np.exp(-1j * kp * L1), 1j * (r3 - omegap) * np.exp(1j * kp * L1)],
                      [np.exp(-1j * kf * Li), np.exp(1j * kf * Li), -np.exp(-1j * kp * Li), -np.exp(1j * kp * Li)],
                      [-r1 * np.exp(-1j * kf * Li), r1 * np.exp(1j * kf * Li), r2 * np.exp(-1j * kp * Li), -r2 * np.exp(1j * kp * Li)]])
        b = np.array([1, 0, 0, 0])
        coef = np.linalg.solve(a, b)
        self.Af = coef[0]; self.Bf = coef[1]; self.Ap = coef[2]; self.Bp = coef[3]
        super().__init__(**kwargs)

    def eval(self, values, x):
        t = self.t
        Li = self.Li
        omegaf = self.omegaf; omegap = self.omegap
        kf = self.kf; kp = self.kp
        Af = self.Af; Bf = self.Bf; Ap = self.Ap; Bp = self.Bp
        if x[0] < Li + 1e-10:
            values[0] = (np.exp(-1j * omegaf * t) * (Af * np.exp(-1j * kf * x[0]) + Bf * np.exp(1j * kf * x[0]))).real
        else:
            values[0] = (np.exp(-1j * omegap * t) * (Ap * np.exp(-1j * kp * x[0]) + Bp * np.exp(1j * kp * x[0]))).real

    def value_shape(self):
        return ()

class u_at_0_harmonic(UserExpression):
    def __init__(self, L0, Li, L1, velf, velp, omegaf, omegap, rhof, rhop, phi, gamma, sigma, **kwargs):
        self.L0 = L0
        self.Li = Li
        self.L1 = L1
        self.velf = velf; self.velp = velp
    ...
    def eval(self, values, x):
        ...
        Af = self.Af; Bf = self.Bf; Ap = self.Ap; Bp = self.Bp
        if x[0] < Li + 1e-10:
            values[0] = (Af * np.exp(-1j * kf * x[0]) + Bf * np.exp(1j * kf * x[0])).real
        else:
            values[0] = (Ap * np.exp(-1j * kp * x[0]) + Bp * np.exp(1j * kp * x[0])).real
...
class v_at_0_harmonic(UserExpression):
    ...
    def eval(self, values, x):
        ...
        Af = self.Af; Bf = self.Bf; Ap = self.Ap; Bp = self.Bp
        if x[0] < Li + 1e-10:
            values[0] = (-1j * omegaf * (Af * np.exp(-1j * kf * x[0]) + Bf * np.exp(1j * kf * x[0]))).real
        else:
            values[0] = (-1j * omegap * (Ap * np.exp(-1j * kp * x[0]) + Bp * np.exp(1j * kp * x[0]))).real

    def value_shape(self):
        return ()

# class PressureTrans(UserExpression):
#     def __init__(self, u, Q, rho, vel, gamma, **kwargs):
#
#         self.u = u
#         self.Q = Q
#         self.rho = rho
...
#         values[0] = -rho * vel**2 / gamma * grad_u(x[0])
#
#     def value_shape(self):
#         return ()
...
probIdStr = 'st-fp-UmnovaLow-P' # Identification of the problem for save files' names
...
phi_porous = 0.5 #0.36 Porosity [0-1]
gamma_porous = 1.4 # Specific heat capacity ratio
sigma_porous = 100. #27888 Flux resistivity [N.s/m^4]
alfa_inf = 1.5 #1.89 Tortuosity
...
omega_fluid = 6 * np.pi * vel_fluid
omega_porous = omega_fluid #6.5 * np.pi * vel_porous
...
def compute(Nele, dt, T_init, T_final, initCondId = 7, savePrefix=''):
    ...
    if initCondId == 1: # Irregular
        ...
        u_at_0 = u_at_0_harmonic(L0=L0, Li=Li0, L1=L1, velf=vel_fluid, velp=vel_porous,
                                 omegaf=omega_fluid, omegap=omega_porous, rhof=rho_fluid, rhop=rho_porous,
                                 phi=phi_porous, gamma=gamma_porous, sigma=sigma_porous, degree=6)
    elif initCondId == 7: # Mexican hat
        #u_at_0 = Expression('a*(1-pow(w*(x[0]-t),2))*exp(-0.5*pow(w*(x[0]-t),2))', a=0.1, w=25, t=0.5*(L0 + L1), degree=6)
    ...
    if initCondId == 6:  # Harmonic
        v_at_0 = v_at_0_harmonic(L0=L0, Li=Li0, L1=L1, velf=vel_fluid, velp=vel_porous,
                                 omegaf=omega_fluid, omegap=omega_porous, rhof=rho_fluid, rhop=rho_porous,
                                 phi=phi_porous, gamma=gamma_porous, sigma=sigma_porous, degree=6)
    else:  # Regular v-2
    ...
    if initCondId == 6:  # Harmonic
        uex = ExactSolutionHarmonic(t=0, L0=L0, Li=Li0, L1=L1, velf=vel_fluid, velp=vel_porous,
                                    omegaf=omega_fluid, omegap=omega_porous, rhof=rho_fluid, rhop=rho_porous,
                                    phi=phi_porous, gamma=gamma_porous, sigma=sigma_porous, degree=6)
    else:
    ...
    zero = Constant("0.0")
    if initCondId == 6:  # Harmonic
        speakerOsc = Expression('cos(omega*t)', omega=omega_fluid, t=0., degree=1)
    elif initCondId == 7:  # Mexican hat
        speakerOsc = Expression('a*(1-pow(w*(t-t0),2))*exp(-0.5*pow(w*(t-t0),2))', a=1, w=15e3, t=0., t0=5e-4, degree=6)
    else:
        speakerOsc = zero
    ...
    a_mass = rho_fluid * inner(u, w) * dx(0) \
             + rho_porous * alfa_inf / phi_porous**2 * inner(u, w) * dx(1)

    # Define the part of the variational problem associated to damping (C)
    a_damping = rho_fluid * vel_fluid * inner(u, w) * ds(3) \
                + sigma_porous / phi_porous * inner(u, w) * dx(1)

    # Define the part of the variational problem associated to stiffness (K)
    a_stiff = rho_fluid * vel_fluid**2 * inner(grad(u), grad(w)) * dx(0) \
              + rho_fluid * vel_fluid**2 / (phi_porous**2 * gamma_porous) * inner(grad(u), grad(w)) * dx(1)
    ...
    csvFile = open('results/%s_Pressures_Nele_%g.csv' % (probIdStr, Nele), 'w', newline='')
    csvWriter = csv.writer(csvFile)
    csvWriter.writerow(["t", "p at 0.2", "p at 0.5", "p at 1"])

    # Loop in time: Newmark method -------------------------------------------------------------------------------------
    for jt in range(Nt + 1):
        # Update time in loads
        force.t = dt * (jt + 1)
        speakerOsc.t = dt * (jt + 1)
        ...
        if jt % Nplot == 0:
            ...
            # eps = 0.1
            #
            # if (np.abs(p_at_02[-1])+np.abs(press(0.2)) > eps) or (setUp02 and (np.abs(press(0.2)) > eps)):
            #     setUp02 = False
            #     tP02 = np.append(tP02, tP02[-1]+dt)
            #     p_at_02 = np.append(p_at_02, press(0.2))
            #
            # if (np.abs(p_at_05[-1]) + np.abs(press(0.5)) > eps) or (setUp05 and (np.abs(press(0.5)) > eps)):
            #     setUp05 = False
            #     tP05 = np.append(tP05, tP05[-1] + dt)
            #     p_at_05 = np.append(p_at_05, press(0.5))
            #
            # if (np.abs(p_at_1[-1]) + np.abs(press(1)) > eps) or (setUp1 and (np.abs(press(1)) > eps)):
            #     setUp1 = False
            #     tP1 = np.append(tP1, tP1[-1] + dt)
            #     p_at_1 = np.append(p_at_1, press(1))

            csvWriter.writerow([dt * (jt + 1), press(0.2), press(0.5), press(1)])

            #timePoints = np.append(timePoints,dt * (jt + 1))
            #p_at_02 = np.append(p_at_02 , press(0.2))
            #p_at_05 = np.append(p_at_05 , press(0.5))
            #p_at_1 = np.append(p_at_1 , press(1))
    ...
    # ax.plot(tP02, p_at_02, label='x = 0.2 m')
    # ax.plot(tP05, p_at_05, label='x = 0.5 m')
    # ax.plot(tP1, p_at_1, label='x = 1 m')
    # ax.set(xlabel='$t$', ylabel='$p$', title='$Pressure$')
    # leg = ax.legend();
    # fig.savefig('figures/%s_Pressure_Nele_%f.png' % (probIdStr, Nele))
    # fig.clf()
...
if __name__ == '__main__':
    ...
    print('Test error: %g' % compute(Nele, dt, T_init, T_final, savePrefix='_del_'))
