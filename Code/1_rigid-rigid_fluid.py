...
class ExactSolutionHarmonic(UserExpression):
    ...
    def eval(self, values, x):
        ...
        B = self.B
        vel = self.vel
        values[0] = 1.E-15*(np.exp(-1j * omega * t) * (A * np.exp(-1j * k * x[0]) + B * np.exp(1j * k * x[0]))).imag
...
class u_at_0_harmonic(UserExpression):
    ...
    def eval(self, values, x):
        ...
        A = self.A
        B = self.B
        values[0] = 1.E-15*(A * np.exp(-1j * k * x[0]) + B * np.exp(1j * k * x[0])).imag
...
class v_at_0_harmonic(UserExpression):
    ...
    def eval(self, values, x):
        ...
        B = self.B
        vel = self.vel
        values[0] = 1.E-15*(-1j * omega * (A * np.exp(-1j * k * x[0]) + B * np.exp(1j * k * x[0]))).imag
...
probIdStr = 'rr-f_harm_' # Identification of the problem for save files' names
...
def compute(Nele, dt, T_init, T_final, initCondId = 4, savePrefix = ''):
    ...
    xdmfFileName = 'results/%s%s_IC_%i_N_%i_dt_%g.xdmf' % (savePrefix, probIdStr, initCondId, Nele, dt)
...
def test_order_h_dt():
    ...
    for dt in [5.83E-05, 2.92E-05, 1.46E-05, 7.29E-06]:
        ...
        error = compute(Nele, dt, T_init, T_final, initCondId=2)
...
if __name__ == '__main__':
    ...
    print('Inital Condition 1 error: %g' % compute(Nele, dt, T_init, T_final, initCondId=4, savePrefix='_DMDShift_'))