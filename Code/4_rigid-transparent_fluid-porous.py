...
probIdStr = 'rt-fp' # Identification of the problem for save files' names
...
# Material interfaces
#Li0 = L1- porous_thickness # Fluid-Porous
#Li0 = 0.6; Li1 = 0.8
Li0 = L1-porous_thickness
...
#porous_domain = CompiledSubDomain("x[0] < Li0", Li0=Li0) # At closer end
#porous_domain = CompiledSubDomain("(x[0] > Li0) && (x[0] < Li1)", Li0=Li0, Li1=Li1) # In the middle
...
#transparent_boundary = CompiledSubDomain("near(x[0], Li0) || near(x[0], Li1)", Li0=Li0, Li1=Li1)
interface = CompiledSubDomain("on_boundary && (near(x[0], Li0))", Li0=Li0)
...
def compute(Nele, dt, T_init, T_final, initCondId = 4, savePrefix=''):
    ...
    if initCondId == 1: # Irregular
        ...
        u_at_0 = Expression('fabs(x[0]-a)<c ? b*(1.-fabs(x[0]-a)/c) : 0.', a=0.5 * (L0 + L1), b=1., c=(L1 - L0) / 50., degree=1)
    else: # Regular v-2
    ...
    interface.mark(boundary_markers, 2) # interface fluid-porous
...
if __name__ == '__main__':
    ...
    print('Test error: %g' % compute(Nele, dt, T_init, T_final, savePrefix='_prev_'))
