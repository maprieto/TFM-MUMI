...
def computeError(approximateSol, exactSol):
    ...
    #return 100*np.linalg.norm(approximateSol.vector() - exactSol.vector(),ord=2)/np.linalg.norm(exactSol.vector(),ord=2)
...
class ExactSolutionHarmonic(UserExpression):
    def __init__(self, t, u0, v0, vel, L0, L1, omega, **kwargs):
        self.t = t
        self.L0 = L0
        self.L1 = L1
        self.u0 = u0
        ...
        self.k = omega / vel
        k = self.k
        self.A = 1. / (np.exp(-1j * k * L0) - np.exp(-1j * k * (2 * L1 - L0)))
        self.B = 1. / (np.exp(1j * k * L0) - np.exp(1j * k * (2 * L1 - L0)))
        super().__init__(**kwargs)
...
class u_at_0_harmonic(UserExpression):
    def __init__(self, vel, L0, L1, omega, **kwargs):
        self.L0 = L0
        self.L1 = L1
        self.vel = vel
        self.omega = omega
...
class v_at_0_harmonic(UserExpression):
    ...
    def eval(self, values, x):
        ...
        B = self.B
        vel = self.vel
        values[0] = 1.E-15*(-1j * omega * (A * np.exp(-1j * k * x[0]) + B * np.exp(1j * k * x[0]))).real
...
probIdStr = 'sr-f_harm_' # Identification of the problem for save files' names

# Parameter values for the fluid
rho_fluid = 1.21  # mass density [kg/m^3]
vel_fluid = 300.  # sound speed [m/s] 343

# Mesh limits (unit interval)
L0 = 0.; L1 = 1.

# omega_harm = 5.5 * np.pi * vel_fluid
omega_harm = 6000
...
def compute(Nele, dt, T_init, T_final, initCondId = 4, savePrefix = ''):
    ...
    if initCondId == 1: # Irregular
        ...
        u_at_0 = u_at_0_harmonic(vel=vel_fluid, L0=L0, L1=L1, omega=omega_harm, degree=6)
    else: # Regular v-2
    ...
    if initCondId == 6: # Harmonic
        v_at_0 = v_at_0_harmonic(vel=vel_fluid, L0=L0, L1=L1, omega=omega_harm, degree=6)
    else: # Regular v-2
        v_at_0 = Expression('0.', degree=1)

    # Initialize exact solution
    if initCondId == 6: # Harmonic
        uex = ExactSolutionHarmonic(t=0, u0=u_at_0, v0=v_at_0, vel=vel_fluid, L0=L0, L1=L1, omega=omega_harm, degree=6)
    else:
    ...
    speaker_boundary.mark(boundary_markers, 2)  # rigid boundary
    ...
    bc_speaker = DirichletBC(V, harmOscilator, boundary_markers, 2)
    ...
    xdmfFileName = 'results/%s%s_IC_%i_N_%i_dt_%g_c_%g.xdmf' % (savePrefix, probIdStr, initCondId, Nele, dt, vel_fluid)
...
if __name__ == '__main__':
    ...
    print('Inital Condition 1 error: %g' % compute(Nele, dt, T_init, T_final, initCondId=6, savePrefix='_DMDMixTest_'))